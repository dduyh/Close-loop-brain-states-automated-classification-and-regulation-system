function [message] = AccuSleep_realtime_viewer_DLE(EEG, EMG, SR, epochLen,epochs,index, userLabels, laser,savepath)
...
%% Check the inputs
 global G
    ...
    if(index == epochs)
        ...
        G.show = epochs; %  default number of bins to display on screen
...
%G.specTs = ((index-5+1):(index-5+G.nbins))*G.epochLen - G.epochLen/2; % spectrogram time axis, in seconds
G.specTs = ((index-epochs+1):(index-epochs+G.nbins))*G.epochLen - G.epochLen/2; % spectrogram time axis, in seconds
G.specTh = G.specTs./3600; % spectrogram time axis, in hours
G.spectrogram = spec(:,showFreqs); % our EEG spectrogram
if nargin == 9 % this probably only happens when called by AccuSleep_GUI
...
if(index == epochs)
    ...
    sampleBins = randperm(G.nbins, G.nbins);
...
caxis(G.A3,G.caxis1);
showFreqs = find(fAxis <= 30); % only show frequencies under 30 Hz
imagesc(G.A3,G.specTs, fAxis(showFreqs), G.spectrogram',G.caxis1);
set(G.A3,'XLim',[min(G.specTs) max(G.specTs)])
axis(G.A3, 'xy')
%axis (G.A3, [min(G.specTs) max(G.specTs)])
%set(G.A3, 'XLimMode','manual', 'YLimMode','manual');
...
%G.A3.XTick = (index-n-0.5)*G.epochLen+tp-(G.show/2)*G.epochLen + G.epochLen*(0:G.show);
G.A3.XTick = min(G.specTs): G.epochLen :(max(G.specTs)+ G.epochLen); % spectrogram time axis, in seconds
ticks = G.A3.XTick;
xlbl = cell(1, length(ticks));
for i = 1:length(ticks)
    xlbl{i} = sec2hr(ticks(i));
end
G.A3.XTickLabel = xlbl;

% plot processed EMG
plot(G.A4,G.specTs,G.cappedEMG,'k')
yr = max(G.cappedEMG) - min(G.cappedEMG); % adjust y limits
set(G.A4,'XTick',[],'YTick',[],'box','off',...
    'XLim',[min(G.specTs) max(G.specTs)],'YLim',[min(G.cappedEMG) - .02*yr, max(G.cappedEMG) + .02*yr])

% Upper sleep stages
box(G.A1, 'on');
xlim(G.A1,[G.specTs(1), G.specTs(end)]);
updateState;
...
%% Functions used by buttons, keypresses, etc.
    function updatePlots(~, ~) % update plots when something changes
        ...
        for i = 1:length(seq)
            if seq(i)==4
                pX = [index-(n+0.5)+x(i)-.5, index-(n+0.5)+x(i)+.5, index-(n+0.5)+x(i)+.5, index-(n+0.5)+x(i)-.5];
                pY = [3.5, 3.5, .5, .5];
        ...
        t = (index-n-0.5)*G.epochLen+tp-N*G.dt:G.dt:(index-n-0.5)*G.epochLen+tp+N*G.dt;
        ...
        % label x axis nicely
        G.A6.XTick = (index-n-0.5)*G.epochLen+tp-(G.show/2)*G.epochLen + G.epochLen*(0:G.show);
        ticks = G.A6.XTick;
        xlbl = cell(1, length(ticks));
        for i = 1:length(ticks)
            xlbl{i} = sec2hr(ticks(i));
    ...
    function updateState() % update the sleep stage image
        ...
        imagesc(G.A1,G.specTs,[1 2 3],makeSleepStageImage(G.labels),[0 4]);
        if G.laserMode
            plot(G.A1, G.specTs, G.laser_by_epoch, 'Color',[.23 .33 1], 'LineWidth', 1,...
                'LineStyle','-');
        end
